...
function AppContent() {
  ...
  // Exclude DiscoverPage, ConnectPage, and InboxPage from Layout to prevent duplicate headers
  const showLayout = !location.startsWith('/auth') && !location.startsWith('/payment-') && !location.startsWith('/stripe/connect') && !location.startsWith('/discover') && !location.startsWith('/connect') && !location.startsWith('/inbox');
